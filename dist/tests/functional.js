...
class FrameNavigationTests extends TurboDriveTestCase {
    ...
    async "test frame navigation with descendant link"() {
        ...
        const [[beforeVisit, beforeVisitTarget, { url }], [visit, visitTarget], [beforeCache, beforeCacheTarget], [beforeRender, beforeRenderTarget, { newBody }], [render, renderTarget], [load, loadTarget, { timing }],] = disptachedFrameEvents;
    ...
    async "test frame navigation with cancelled visit"() {
        ...
        await this.nextBeat;
        const disptachedFrameEvents = await this.readDispatchedFrameEvents();
        const [[beforeVisit, beforeVisitTarget, { url }], ...others] = disptachedFrameEvents;
        this.assert.equal(beforeVisit, "turbo:before-frame-visit");
        this.assert.equal(beforeVisitTarget, "frame");
        this.assert.ok(url.includes("/src/tests/fixtures/frame_navigation.html"));
    ...
    async trackFrameEvents() {
        this.remote.execute((eventNames) => {
            var _a;
            const html = document.documentElement;
            const element = (_a = document.getElementById("frame")) === null || _a === void 0 ? void 0 : _a.parentNode;
            if (element) {

...
class TurboDriveTestCase extends FunctionalTestCase {
    ...
    async setLocalStorageFromEvent(event, key, value) {
        return this.remote.execute((eventName, storageKey, storageValue) => {
            addEventListener(eventName, () => localStorage.setItem(storageKey, storageValue));
        }, [event, key, value]);
    }
    getFromLocalStorage(key) {
        return this.remote.execute((storageKey) => localStorage.getItem(storageKey), [key]);
    }
    get nextBody() {
        return (async () => {
            let body;
...
class FrameTests extends TurboDriveTestCase {
    ...
    async "test removing [disabled] attribute from eager-loaded frame navigates it"() {
        await this.remote.execute(() => { var _a; return (_a = document.getElementById("frame")) === null || _a === void 0 ? void 0 : _a.setAttribute("disabled", ""); });
        await this.remote.execute(() => { var _a; return (_a = document.getElementById("frame")) === null || _a === void 0 ? void 0 : _a.setAttribute("src", "/src/tests/fixtures/frames/frame.html"); });
...
class RenderingTests extends TurboDriveTestCase {
    ...
    async "test reloads when tracked elements change"() {
        await this.remote.execute(() => window.addEventListener("turbo:reload", (e) => {
...
class VisitTests extends TurboDriveTestCase {
    ...
    async getDocumentElementAttribute(attributeName) {
        return await this.remote.execute((attributeName) => document.documentElement.getAttribute(attributeName), [attributeName]);
    }
}
